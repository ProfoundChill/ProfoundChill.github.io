...
// --- Part 4: Anti-Brute Force SETUP (Temporary Session-Based Storage) ---
$lockout_duration = 30; // 30 seconds
$max_attempts = 3; 

// Initialize the lockout array within the session if it doesn't exist
// Key is the username, Value is {attempts: X, locked_until: Y}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['logout'])) { 
    ...
    // IMPORTANT: When session_destroy() runs, the lockout tracking is also reset.
} 
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['password'], $_POST['username'])) {
    ...
    if ($user_attempts['locked_until'] > time()) { 
        ...
        $error = "Account locked for 30 seconds. Remaining time: {$time_remaining}s";
    }
    ...
    // NOTE: No file_put_contents needed. Session is automatically saved by PHP.
}
?>
